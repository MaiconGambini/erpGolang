...
| `e2e.yml` | `frontend/**`, `backend/**` | migrate, seed, Playwright (21 tests) |
...
- VPS deploy automation exists (`deploy-vps.yml`) but needs `VPS_SSH_KEY`/`VPS_HOST`/`VPS_USER`/`VPS_DOMAIN`/`VPS_FINGERPRINT` secrets configured before first run; backup/restore scripts shipped (`deploy/scripts/`), first live drill still pending.
