...
(
  cd backend
  go run ./cmd/migrate
  go run ./cmd/seed   # tenants acme/beta, password admin123 (local development only)
)
...
| Backend integration | `(cd backend && go test -tags=integration ./internal/customers/... ./internal/dashboard/... ./internal/sales/... ./internal/reports/...)` | exit 0 (needs `DATABASE_URL` + seed) |
| Frontend | `(cd frontend && npm run typecheck && npm run test:unit && npm run build)` | exit 0 |
| E2E | `(cd frontend && npx playwright test)` | 21 tests green (dockerized API + DB) |
...
Low-stock threshold `5` is centralized in Go (`backend/shared/inventory`) and TypeScript (`frontend/src/shared/config/inventory`).
...
Sales confirm/cancel use conditional status updates and `SELECT ... FOR UPDATE`; a losing concurrent confirm returns `INVALID_STATUS` (409) before touching product rows. Confirm/cancel remain non-idempotent; see `backend/docs/SALES_TRANSACTIONS.md`.
