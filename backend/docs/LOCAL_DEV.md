...
(cd backend && go run ./cmd/seed)    # tenants acme/beta; admin@*.com + viewer@acme.com / admin123 (local development only)
(cd backend && sqlc generate) # after query changes
```

## 4. Run API

```bash
make backend-dev             # air via .air.toml
# or:
...
(cd frontend && npx playwright test)  # 21 tests; Vite on :5174; defaults :5434/:6381 (see playwright.config.ts)
```

## Manual Smoke

1. Login as `acme` / `admin@acme.com` / `admin123` (local development only)
...
5. Viewer smoke: `viewer@acme.com` / `admin123` (local development only) — no create buttons; `/users` redirects
