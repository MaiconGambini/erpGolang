...
func (h *Handler) List(w http.ResponseWriter, r *http.Request) {
	...
	if q.Get("format") == "csv" {
		...
		if err != nil {
			httpx.Error(w, "INTERNAL_ERROR", "failed to export suppliers", http.StatusInternalServerError)
			return
		}
		...
		for _, s := range items {
			...
			rows = append(rows, []string{s.Name, doc, email, active})
		}
		_ = export.WriteCSV(w, "fornecedores.csv", []string{"nome", "documento", "email", "ativo"}, rows)
		return
	}
	...
	if err != nil {
		httpx.Error(w, "INTERNAL_ERROR", "failed to list suppliers", http.StatusInternalServerError)
		return
	}
...
func (h *Handler) Get(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		httpx.Error(w, "INTERNAL_ERROR", "failed to get supplier", http.StatusInternalServerError)
		return
	}
...
func (h *Handler) Delete(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.svc.Delete(r.Context(), tenantID, user.ID, id); err != nil {
		...
		httpx.Error(w, "INTERNAL_ERROR", "failed to delete supplier", http.StatusInternalServerError)
		return
	}
